...
- **Approval**: one admin action (`approveProvider`), approve-only. No reject/suspend/deactivate action exists despite `ProviderStatus` already having `SUSPENDED`/`DEACTIVATED` values.
...
- **Full onboarding review workflow**: reject, suspend, deactivate, re-approve — not just the current one-way approve.
