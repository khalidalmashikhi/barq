...
- **How should `Service` relate to `Category`/`SubCategory`?** Proposed, not yet approved (2026-07-26): `ADR-0014-service-category-relational-taxonomy.md` (Draft) recommends a single nullable `Service.categoryId` + optional nullable `subCategoryId`, with the keyword-match bridge shipped in the UX/Navigation Remediation remaining as a documented temporary stand-in until that ADR and its implementation plan (`../plans/RELATIONAL-SERVICE-TAXONOMY-PLAN.md`) are approved. See `07-CATEGORIES.md`.
