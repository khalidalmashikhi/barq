...
**Enforcement status: Enforced at publish time (Task B, 2026-08-06).** `Service.categoryId` is a single nullable FK (`onDelete: Restrict`) to the most-specific `Category` node. Existing services may remain `categoryId = NULL`; DRAFT/PAUSED may stay uncategorized. The publish transition (provider + admin) now blocks an uncategorized service via the shared `assertServicePublishable`, mirroring the existing "no publish without an ACTIVE Price" rule — never a `NOT NULL` database constraint. Forward-only: already-PUBLISHED uncategorized services are not retroactively unpublished. See `ADR-0015`, `ADR-0014`.
...
**Enforcement status: Partially enforced (Task B, 2026-08-06); full reconciliation P4.** Once `Service.categoryId` is set, the service's vertical (`Service.serviceType` CTI discriminator) must be consistent with `Category.serviceTypeKey`. Task B prevents the inconsistency from being *introduced* through the write path: both the selectable-category query and the server-side `assertAssignableCategory` validator scope eligible categories to the service's own `serviceType`, so a mismatched assignment is rejected. Reconciling pre-existing rows and handling a later `serviceType`/re-parent change remains P4 work. See `ADR-0015`.

### BR-029 — A provider may only be approved once every required verification document is APPROVED
**Enforcement status: Enforced server-side (Provider Verification & Documents, Gate 3, 2026-08-10).** `approveProvider()` calls `assertProviderApprovable(providerId)` before any transition: if a required document (per `requiredDocumentTypesFor` — `IDENTITY_PROOF` for INDIVIDUAL, `COMMERCIAL_REGISTRATION` for COMPANY) is missing or not `APPROVED`, it returns `INCOMPLETE_DOCUMENTS` with the ordered blockers and performs **no** status change, audit, or notification. The optional `TOURISM_LICENCE` and non-document fields (logo/cover/portfolio/categories/description) never block. Enforcement is server-authoritative; the admin UI's blocker panel + disabled Approve button only reflect it. **Grandfathering:** the gate runs only on an `APPLIED`/`UNDER_REVIEW` → `APPROVED` transition, so existing APPROVED providers are never retroactively re-verified, suspended, or forced to upload. Replacing an already-APPROVED document returns it to `PENDING` but the provider stays APPROVED/operational. Document-level rejection is independent of provider-level rejection (rejecting a document does **not** reject the whole provider). See `17-CHANGELOG-DECISIONS.md` (Gate 3), `15-DATA-DICTIONARY.md` (ProviderDocument).
