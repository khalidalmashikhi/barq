...
**Relationships:** Has many `SubCategory`. Does not yet relate to `Service` — a relational `Service.categoryId` is proposed, not approved, in `ADR-0014-service-category-relational-taxonomy.md` / `../plans/RELATIONAL-SERVICE-TAXONOMY-PLAN.md` (2026-07-26). Today's homepage/dashboard category navigation uses a temporary keyword-match filter instead (see `Service` entry below).
...
**Planned extensions:** `Service.categoryId` assignment (proposed, not approved — see `ADR-0014`); customer-facing Discovery/Search (Marketplace, later phases).
...
**Description:** `serviceType` (string CTI discriminator), `name`/`description` (bilingual `Json`), `status` (`ServiceStatus`: `DRAFT`/`PUBLISHED`/`PAUSED`/`ARCHIVED`).
...
**Planned extensions:** Relational category/subcategory assignment (`categoryId`/`subCategoryId`) — proposed, not approved, in `ADR-0014-service-category-relational-taxonomy.md` / `../plans/RELATIONAL-SERVICE-TAXONOMY-PLAN.md`; gallery/image support (no image model exists on Service today).
...
**Description:** Modeled as `Price` — `amount` (Decimal 12,2), `currency`, `status` (`ACTIVE`/`SUPERSEDED`). Immutable/append-only: a price change creates a new row.
