...
- **Status:** Accepted. Schema/design gate landed in `1c36d22`. The **runtime-wiring + default-seed gate** is now implemented (staging bootstrap for the default rows, fail-closed policy reads wired into `assertProviderApprovable` / the provider verification checklist / upload type validation). The **Admin CRUD** management UI remains deferred to a follow-up gate. The staging seed is created by a guarded, idempotent, dry-run-by-default bootstrap (`scripts/bootstrap-staging-verification-requirements.ts`) and has NOT been applied yet.
