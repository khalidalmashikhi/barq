...
describe("getCustomers", () => {
  ...
  it("filters by User.status (the customer's access status) when provided", async () => {
    requireAdminMock.mockResolvedValue({ admin: { id: "admin-1" } });
    countMock.mockResolvedValue(0);
    findManyMock.mockResolvedValue([]);

    await getCustomers({ status: "DEACTIVATED" });

    expect(countMock).toHaveBeenCalledWith({ where: { user: { status: "DEACTIVATED" } } });
  });
