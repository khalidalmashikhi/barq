...
export async function evaluateRentalServicePublishable(
  ...
  // vehicle/offering simply does not match — non-enumerating).
  ...
  for (const offering of offerings) {
    // Vertical compliant + vehicle selectable + verified capacity (identical authority to C2b-R publish).
    if ((await assertRentalPublishReady(db, service.providerId, offering.vehicle, now)) !== null) continue;
    // Authoritative money/currency/capacity re-validation (fail-closed, independent of the write path).
    ...
    // >= 1 OPEN non-past day resolving a positive daily rate.
