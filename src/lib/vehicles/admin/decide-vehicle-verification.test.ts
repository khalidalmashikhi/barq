...
describe("approveVehicleVerification — two-axis guarantee", () => {
  it("SUBMITTED → APPROVED when ready; sets review metadata; NEVER writes Asset.status", async () => {
    ...
    assetFindFirstMock.mockResolvedValue({ verificationStatus: "SUBMITTED", vehicle: { assetId: "asset-1" }, documents: READY_DOCS, provider: { userId: "user-1" } });
    ...
    // §18/§3 — provider notified, recipient server-derived (Asset -> Provider -> userId).
    expect(notifyMock).toHaveBeenCalledWith("vehicle.verification_approved", { providerUserId: "user-1", assetId: "asset-1" });
  });
  ...
  it("returns NOT_SUBMITTED when the vehicle is not SUBMITTED", async () => {
    requireAdminMock.mockResolvedValue({ admin: { id: "admin-1" } });
    assetFindFirstMock.mockResolvedValue({ verificationStatus: "DRAFT", vehicle: { assetId: "asset-1" }, documents: READY_DOCS, provider: { userId: "user-1" } });
    expect(await approveVehicleVerification("asset-1")).toEqual({ ok: false, error: "NOT_SUBMITTED" });
  });

  it("§16 — refuses to approve when a required document is already EXPIRED (NOT_READY, no notify)", async () => {
    requireAdminMock.mockResolvedValue({ admin: { id: "admin-1" } });
  ...
  it("treats a lost race (updateMany count 0) as NOT_SUBMITTED, no audit", async () => {
    ...
    // §17 idempotency — a raced/no-op transition emits NO notification.
    expect(notifyMock).not.toHaveBeenCalled();
  });
...
describe("rejectVehicleVerification / requestVehicleChanges", () => {
  it("SUBMITTED → REJECTED with a reason; never writes Asset.status", async () => {
    requireAdminMock.mockResolvedValue({ admin: { id: "admin-1" } });
    assetFindFirstMock.mockResolvedValue({ verificationStatus: "SUBMITTED", provider: { userId: "user-1" } });
    txUpdateManyMock.mockResolvedValue({ count: 1 });
    ...
    expect(notifyMock).toHaveBeenCalledWith("vehicle.verification_rejected", { providerUserId: "user-1", assetId: "asset-1" });
  });
  ...
  it("SUBMITTED → CHANGES_REQUESTED with a reason; never writes Asset.status", async () => {
    ...
    expect(notifyMock).toHaveBeenCalledWith("vehicle.changes_requested", { providerUserId: "user-1", assetId: "asset-1" });
  });
