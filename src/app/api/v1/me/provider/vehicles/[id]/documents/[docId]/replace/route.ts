...
// POST /api/v1/me/provider/vehicles/{id}/documents/{docId}/replace — VEHICLE-LC2B.
//
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; docId: string }> }) {
  return withRequestTracing("api.v1.me.provider.vehicles.documents.replace", () =>
    withApiV1ProviderMutation(request, async ({ locale }) => {
      ...
      const result = await replaceVehicleDocument(vehicleId, docId, { ...parsed.file, claimedExpiryDate: parsed.claimedExpiryDate });
