...
export function DashboardHero() {
  return (
    <motion.div
      ...
      style={{ height: 420 }}
    >
      <DestinationImage src={DESTINATION_IMAGES.salalah} alt="عُمان" className="absolute inset-0" />
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
        <h1 className="max-w-2xl text-3xl font-semibold text-white sm:text-4xl">
          اكتشف سلطنة عمان بطريقة مختلفة
        </h1>
        <p className="mt-3 text-lg text-white/75">أفضل التجارب السياحية المختارة بعناية</p>
        ...
        <div className="mx-auto mt-8 flex w-full max-w-2xl items-center gap-2 rounded-full bg-white p-2.5 shadow-premium-lg">
          ...
          <button type="button" className="hidden items-center gap-1.5 px-3 text-sm text-foreground/60 sm:flex" aria-label="الموقع">
            <MapPin size={16} strokeWidth={1.75} />
            الموقع
          </button>
          <button type="button" className="hidden rounded-full p-2.5 text-foreground/60 hover:bg-background sm:flex" aria-label="تصفية">
