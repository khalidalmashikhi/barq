...
// Service admin filters — Phase 2.4 (Service Admin UI). Mirrors
// provider-filters.tsx exactly: plain GET-form, no client JS, real
// removable filter chips — this codebase's established search/filter
// convention, not a new one.

const SERVICE_STATUSES = ["DRAFT", "PUBLISHED", "PAUSED", "ARCHIVED"] as const;
...
export async function ServiceFilters({ currentSearch, currentStatus }: ServiceFiltersProps) {
  const t = await getServerTranslator("admin");
  const locale = await getLocale();
  const basePath = getPathname({ href: "/admin/services", locale });
  ...
  if (currentStatus) {
    chips.push({ key: "status", label: t(getServiceStatusTranslationKey(currentStatus)), href: hrefWithout("status") });
  }
          ...
          >
            ...
            {SERVICE_STATUSES.map((status) => (
              <option key={status} value={status}>
                {t(getServiceStatusTranslationKey(status))}
              </option>
            ))}
          </select>
        </label>
